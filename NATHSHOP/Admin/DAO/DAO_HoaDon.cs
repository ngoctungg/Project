...
namespace NATHSHOP.Admin
{
    class DAO_HoaDon:QuanLyKetNoi
    {
        ...
        public DataTable LoadDSHD(bool TrangThai)
        {
            ...
            dao.Open();
            string qry = "select MaHD,NgayLapHD,NgayGH,HoTen,DiaChiGiaoHang,TrangThai from HoaDon h,KhachHang k where h.MaKH = k.MaKH and h.TrangThai = '" + TrangThai + "'";
        ...
        public HoaDon LayThongTinHD(int MaHD)
        {
            DAO_HoaDon dao = new DAO_HoaDon();
            dao.Open();
            string qry = "select MaHD,HoTen,NgayLapHD,NgayGH,DiaChiGiaoHang,TrangThai from HoaDon h,KhachHang k where h.MaKH = k.MaKH and h.MaHD =" + MaHD;
            SqlCommand cmd = new SqlCommand(qry, dao.cnn);
            SqlDataReader reader = cmd.ExecuteReader();
            reader.Read();
        ...
        public DataTable TinhDoanhThu()
        {
            ...
            DAO_HoaDon dao = new DAO_HoaDon();
            dao.Open();
            string qry = "select sum(A.GiaMua*B.SoLuong) as TongMua ,sum(B.DonGia*B.SoLuong) as TongBan , SUM((B.DonGia*B.SoLuong) - (A.GiaMua*B.SoLuong)) as DoanhThu from SanPham A,ChiTietHoaDon B where A.MaSP = B.MaSP";
            SqlCommand cmd = new SqlCommand(qry, dao.cnn);
            SqlDataReader reader = cmd.ExecuteReader();
